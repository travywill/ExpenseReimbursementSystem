...
function listPersonalPendingRequests() {
	...
	fetch("http://localhost:8082/ExpenseReimbursementSystem//employeepersonalpendingrequests").then(function(response) {
		...
		if(data.session === null) {
			...
			for (i = 0; i < requestArray.length; i++) {
				...
				document.getElementById("employeepersonalpendingrequests").appendChild(li);
			}
